namespace CLRBandicoot.MIPS
{
    public abstract class Instruction
    {
        public static Instruction Disassemble(int value)
        {
            ...
            switch ((Opcode)opcode)
            {
                ...
                case Opcode.NativeCode:
                    return new NativeCodeInstruction(jumptarget);
            }
        ...
        public virtual int Execute(Instruction next)
        {
            throw new PSXException();
        }

        public virtual void Mark(Compiler c,int address)
        {
            c.MarkDirect(address + 4);
        }

        public abstract bool Compile(Compiler c,int address,bool direct);

        /*protected bool CompileDefault(Compiler c,int address,bool direct)
        {
            c.IL.Emit(System.Reflection.Emit.OpCodes.Ldc_I4,address);
            c.IL.Emit(System.Reflection.Emit.OpCodes.Call,typeof(PCSX).GetMethod("ExecuteOnce"));
            return true;
        }

        protected bool CompileDefaultX(Compiler c,int address,bool direct)
        {
            if (direct)
            {
                c.IL.Emit(System.Reflection.Emit.OpCodes.Ldc_I4,address);
                c.IL.Emit(System.Reflection.Emit.OpCodes.Call,typeof(PCSX).GetMethod("ExecuteOnce"));
                c.IL.Emit(System.Reflection.Emit.OpCodes.Call,typeof(PSX).GetProperty("PC").GetGetMethod());
                c.IL.Emit(System.Reflection.Emit.OpCodes.Ret);
                return false;
            }
            else
            {
        ...
        protected bool CompileDefaultBranch(Compiler c,int address,bool direct)
        {
            if (!direct)
                throw new PSXException();
            c.IL.Emit(System.Reflection.Emit.OpCodes.Ldc_I4,address);
